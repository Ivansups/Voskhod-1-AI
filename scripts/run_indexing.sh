...
# Скрипт для запуска индексации документов RAG системы
# Использует curl вместо Python зависимостей

echo "🚀 Запуск индексации документов для RAG системы"
echo "=================================================="
...
if echo "$status_response" | jq . > /dev/null 2>&1; then
    ...
    echo "📊 Текущий статус: фаза=$phase, документов=$documents_count"

    if [ "$phase" = "ready" ] && [ "$documents_count" -gt 1000 ]; then
        echo "✅ Данные уже актуальные, запускаем инкрементальную индексацию"
        force_param=""
        step_num="2️⃣"
    else
        echo "⚠️  Данные не актуальные или система не готова, выполняем полную индексацию"
        echo ""
        echo "2️⃣ Очистка старой коллекции..."
    ...
    echo "❌ Не удалось получить статус, выполняем полную индексацию"
    echo ""
    echo "2️⃣ Очистка старой коллекции..."
    curl -s -X DELETE "$QDRANT_URL/collections/university_knowledge" > /dev/null
    if [ $? -eq 0 ]; then
...
echo ""
echo "${step_num} Запуск индексации..."
response=$(curl -s -X POST "$API_BASE_URL/v1/admin/sync$force_param")
if echo "$response" | grep -q "running"; then
    mode=$(echo "$response" | jq -r '.force_reindex')
    if [ "$mode" = "true" ]; then
        echo "✅ Полная индексация запущена в фоне"
    else
        echo "✅ Инкрементальная индексация запущена в фоне"
    fi
else
